...
def main(args: List[str]):
    ...
    parser.add_argument("--size", type=int, help="size", default=int(2e6))
    parser.add_argument("--tr", type=float, help="treatment_ratio", default=0.5)
    parser.add_argument("--comp", type=float, help="compliances", default=1)
    parser.add_argument("--cATE", type=float, help="ctrl_outcome_rate", default=0.1)
    parser.add_argument("--n_splits", type=int, help="n_splits", default=51)
    parser.add_argument("--seed", type=int, help="seed", default=338)
    parser.add_argument("--output", type=str, help="Path to the output directory")
    # parser.add_argument("model", help="Uplift model for pred")
    # parser.add_argument("-n", "--ncpus", type=int, default=1, help="Number of CPUs to be used")
    # args = parser.parse_args(args[1:])
    args = parser.parse_args()
    data_gen = SyntheticDataGenerator(
            size=args.size,
            treatment_ratio=args.tr,
            compliances=args.comp,
            ctrl_outcome_rate=args.cATE,
            treatment_uplifts=treatment_uplifts2,
            n_splits=args.n_splits,
            seed=args.seed
        )
    output_directory = os.path.abspath(args.output)
    # model_used = data_gens[args.model]
    # n_cpus = args.ncpus

    if not os.path.isdir(output_directory):
        os.makedirs(output_directory)
    prefix_output_filename = os.path.join(output_directory, f"resultCustom_tr{args.tr}_cATE{args.cATE}_comp{args.comp}")
